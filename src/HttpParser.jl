...
mutable struct ParserSettings
    on_message_begin_cb::Ptr{Compat.Cvoid}
    on_url_cb::Ptr{Compat.Cvoid}
    on_status_complete_cb::Ptr{Compat.Cvoid}
    on_header_field_cb::Ptr{Compat.Cvoid}
    on_header_value_cb::Ptr{Compat.Cvoid}
    on_headers_complete_cb::Ptr{Compat.Cvoid}
    on_body_cb::Ptr{Compat.Cvoid}
    on_message_complete_cb::Ptr{Compat.Cvoid}
    on_chunk_header::Ptr{Compat.Cvoid}
    on_chunk_complete::Ptr{Compat.Cvoid}
end
...
function http_parser_init(parser::Parser,isserver=true)
    ccall((:http_parser_init, lib), Compat.Cvoid, (Ptr{Parser}, Cint), Ref(parser), !isserver)
end
...
"Pauses the parser."
pause(parser::Parser) = ccall((:http_parser_pause,lib), Compat.Cvoid, (Ptr{Parser}, Cint), Ref(parser), one(Cint))
"Resumes the parser."
resume(parser::Parser) = ccall((:http_parser_pause,lib), Compat.Cvoid, (Ptr{Parser}, Cint), Ref(parser), zero(Cint))
